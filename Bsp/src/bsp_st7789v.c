...
static uint8_t spi_write_byte(uint8_t txdata,uint16_t size);
//static uint8_t lcd_buf[LCD_Buf_Size];

static void tft_write_data1(uint8_t dat1,uint8_t dat2);
...
/*******************************************************************************
 * 
 * Function Name: void tft_gpio_init(void)
 * Function : 
 * Input Ref: 
 * Return Ref
 * 
...
void tft_gpio_init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOB, BACKLIGHT_Pin|CMD_DIP_Pin|TFT_RESET_Pin|CS_Pin, GPIO_PIN_RESET);


  /*Configure GPIO pins : PBPin PBPin PBPin PBPin */
  GPIO_InitStruct.Pin = BACKLIGHT_Pin|CMD_DIP_Pin|TFT_RESET_Pin|CS_Pin;
...
/*******************************************************************************
 * 
 * Function Name: uint8_t spi_write_byte(uint8_t *txdata,uint16_t size)
...
static uint8_t spi_write_byte(uint8_t txdata,uint16_t size)
{
   ...
   // __HAL_SPI_CLEAR_FREFLAG(&hspi2);
	//return HAL_SPI_Transmit_DMA(&hspi2,&txdata,1);
    return HAL_SPI_Transmit(&hspi2,&txdata,1,0xffffffff);
...
{
  //  LCD_NSS_SetLow();
    TFT_DCX_CMD(); //hight level
    spi_write_byte(cmd,1); //write "1" 
...
void LCD_Write_16bit_Cmd(uint16_t cmd)
{
    LCD_Write_Cmd(cmd >>8);
    LCD_Write_Cmd(cmd);
}
...
//		}
		...
		for(j=0;j<240;j++)
		{
			...
			if(p<4440 && j< 121){
			//tft_write_data1(gImage_s07_main_picture[p],gImage_s07_main_picture[p+1]);
		//	tft_write_data1(gImage_s07_temp[p],gImage_s07_temp[p+1]);
			p++;
			q++;
